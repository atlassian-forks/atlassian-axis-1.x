...
public class UnionServiceTestCase extends TestCase {
    ...
    public void testBasicUnion1() throws Exception {
        ...
        FooOpenEnum enumElement = null;

        String testStrURI = "http://foobar/A";
        enumElement = new FooOpenEnum(testStrURI);
        assertEquals(testStrURI, enumElement.toString());

        URI testURI = new URI(testStrURI);
        enumElement = new FooOpenEnum(testURI);
        assertEquals(testStrURI, enumElement.toString());
        assertEquals(testURI, enumElement.getAnyURIValue());

        enumElement = new FooOpenEnum(FooEnum.value1);
        assertEquals(FooEnum.value1.toString(), enumElement.toString());
        assertEquals(FooEnum.value1, enumElement.getFooEnumValue());
        assertEquals(FooEnum._value1, enumElement.getAnyURIValue());
    }
