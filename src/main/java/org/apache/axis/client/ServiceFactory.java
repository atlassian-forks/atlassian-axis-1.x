    ...
    public static Service getService(Map environment)
    {
        ...
        if (context != null) {
            ...
            if (name != null && (name.toUpperCase().indexOf("LDAP") != -1
                    ...
                    || name.toUpperCase().indexOf("IIOP") != -1
                    || name.toUpperCase().indexOf("CORBANAME") != -1)) {
                log.warn("returning null, jndiName received by ServiceFactory.getService() "
                        + "is not supported by this method: " + name);
                return null;
            }
            if (name == null) {
